...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const dots = [...dotsContainer.children]; // We add an event listener on the parent container to the dots.

  dotsContainer.addEventListener('click', _ref => {
    let {
      target
    } = _ref;
    // If the item clicked was not a dot, don't do anything
    if (!target.matches('.testimonial-dot')) return;
    const myIndex = dots.indexOf(target); // Move the container -100% * the dot index
